...
public class DataService {
    ...
    @PostConstruct
    void seed() {
        int p1 = addPost("Primer post: cuidados de tu mascota", "Contenido A", LocalDate.now().minusDays(3));
        int p2 = addPost("Segundo post: alimentación saludable", "Contenido B", LocalDate.now().minusDays(2));
        int p3 = addPost("Tercer post: juegos y enriquecimiento", "Contenido C", LocalDate.now().minusDays(1));
    ...
    public List<Post> searchPosts(String query) {
        ...
        return posts.stream()
                .filter(p -> p.getTitle().toLowerCase().contains(q) || p.getContent().toLowerCase().contains(q))
                .collect(Collectors.toList());
    }
    ...
    public List<Comment> getCommentsByPost(int postId) {
        ...
        return comments.stream().filter(c -> c.getPostId() == postId).collect(Collectors.toList());
    }
    ...
    public int addComment(int postId, String author, String text, LocalDate date) {
        ...
        if (text == null || text.isBlank()) throw new IllegalArgumentException("text requerido");
        if (date == null) throw new IllegalArgumentException("date requerido");
    ...
    public List<Rating> getRatingsByPost(int postId) {
        ensurePostExists(postId);
        return ratings.stream().filter(r -> r.getPostId() == postId).collect(Collectors.toList());
    }

    public int addRating(int postId, int value) {
        ensurePostExists(postId);
        if (value < 1 || value > 5) throw new IllegalArgumentException("value debe estar entre 1 y 5");
    ...
    public RatingSummary getRatingSummary(int postId) {
        ...
        int count = list.size();
        double avg = count == 0 ? 0.0 : list.stream().mapToInt(Rating::getValue).average().orElse(0.0);
